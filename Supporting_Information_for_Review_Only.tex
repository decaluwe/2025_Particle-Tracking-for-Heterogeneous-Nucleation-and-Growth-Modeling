...
\textcolor{red}{The average error in Table~\ref{Table} represents the average percent mismatch at radius $r_i$, relative to algorithm 3, using the a sum of squared residuals:}
\begin{equation}\label{eq:psd_error}
    \textcolor{red}{Err_j = \frac{1}{n_{\rm bins}}\sum_{i=0}^{n_{\rm bins}} \left(\frac{N_{i,j} - N_{i,3}}{N_{i,3}}\right)^2,}
\end{equation}
\textcolor{red}{where $N_{i,j}$ is the number density (particles per unit area) of deposited particles having radis $r_i$ using algorithm $j$. For radii where $N_{i,3}=0$, but $N_{i,j}\neq 0$ for algorithm 1 or 2, the local error contribution is set to 100\% to avoid division by zero. It is noteworthy in Table~\ref{Table} that algorithms 1 and 2 give the same average error.  This reflects the fact that the `bookmarking' in algorithm 2 does not prevent so-called diffusion between radial bins; it only constrains it, so that particles `pile up' at the leading bookmark.  The average error is proportional to the area between the algorithm's PSD and that of algorithm 3.  Visual inspection of Figure~\ref{fig:Sup_PSDs} shows roughly equal areas for algorithms 1 and 2 on either side of the maximum radius from algorithm 3, consistent with the results in Table~\ref{Table}.}


\textcolor{red}{Although algorithm 1 matches the overall capacity from the reference publication, the differences in PSD will have increasing impact as simulations are used to predict extended cycling in realistic situations. For example, if the battery is not fully recharged (i.e., deposits not fully removed) before charging resumes, the particles at the low-diameter end of the PSD will erode, but the high-diameter portion of the PSD would remain. The shape of the PSD when discharge resumes will impact the available surface area for growth, the resulting degree of supersaturation, and the particle sizes that develop during discharge, impacting subsequent battery performance.}
